read2_paths <-
function(ordered_alldata=ordered_alldata,gs_locs=gs_locs,pathinfo="",sets_prefix=""){
ps <- list.files(pattern=sets_prefix)
pathways <- as.matrix(read.table(pathinfo,sep="\t",quote="\"",header=T))
...
genes <- as.numeric(read.table(ps[i],header=F,sep=" "))
